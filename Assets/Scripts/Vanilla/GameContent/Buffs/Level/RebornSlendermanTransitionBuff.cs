...
{
    [BuffDefinition(VanillaBuffNames.Level.rebornslendermanTransition)]
    public class RebornSlendermanTransitionBuff : BuffDefinition
    {
        public RebornSlendermanTransitionBuff(string nsp, string name) : base(nsp, name)
        {
        ...
        public override void PostUpdate(Buff buff)
        {
            ...
            if (timeout == CREATE_DARKNESS_TIMEOUT)
            {
                ...
                // 音乐。
                level.PlayMusic(VanillaMusicID.rebornmareBoss);
                level.SetMusicVolume(1);
            }
            else if (timeout <= 0)
            {
                Vector3 pos = new Vector3(level.GetEntityColumnX(4), 0, level.GetEntityLaneZ(2));
                var boss = level.Spawn(VanillaBossID.rebornslenderman, pos, null);
