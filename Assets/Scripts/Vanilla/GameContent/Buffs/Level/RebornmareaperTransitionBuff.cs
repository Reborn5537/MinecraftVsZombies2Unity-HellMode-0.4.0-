...
{
    [BuffDefinition(VanillaBuffNames.Level.rebornmareaperTransition)]
    public class RebornNightmareaperTransitionBuff : BuffDefinition
    {
        public RebornNightmareaperTransitionBuff(string nsp, string name) : base(nsp, name)
        {
        ...
        public override void PostUpdate(Buff buff)
        {
            ...
            {
                // 梦魇收割者出现
                level.PlayMusic(VanillaMusicID.rebornmareBoss2);
                // Create Boss
                Vector3 pos = new Vector3(level.GetEntityColumnX(4), 0, level.GetEntityLaneZ(2));
                var boss = level.Spawn(VanillaBossID.rebornnightmareaper, pos, null);
                RebornNightmareaper.Appear(boss);
